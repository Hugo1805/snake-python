...
def pantalla(stdscr):
    ...
    #motrar menu en pantalla
    print_menu(stdscr, current_row)

    while 1:
        #compatimiento del cursor
        key = stdscr.getch()
...
def main(stdscr):
    ...
    #DELIMITAMOS AREA DEL JUEGO
    sh, sw = stdscr.getmaxyx()
    box = [[3,3], [sh-3, sw-3]]
    stdscr.addstr(1, 81, "'q'=QUIT  <SPACE BAR>PAUSE/CONTINUE")
    textpad.rectangle(stdscr, box[0][0], box[0][1], box[1][0], box[1][1])

    #CREAMOS SERPIENTE Y DEFINIMOS MOVIENTOS INICIAL
    ...
    while 1:
        ...
        #Pause juego y mostrar 'pause' en patalla.
        if key == ord(' '):
            if PAUSE == False:
                PAUSE = True
                init_pausesound()
        ...
        if PAUSE == False:
            ...
            stdscr.addstr(new_head[0], head[1], '#')
